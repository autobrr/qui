...
| `sessionSecret` | `QUI__SESSION_SECRET` / `QUI__SESSION_SECRET_FILE` | string | auto-generated | WARNING: a changed value breaks decryption of stored instance passwords. You must enter them again in the UI. Use at least 32 characters on a new install, and leave the value alone on an install that already stores credentials. On the first start after upgrading, qui re-encrypts stored credentials under a key derived from this secret. That re-encryption is one way. An older qui cannot read the new values, so after rolling back you must enter the credentials again or restore a database backup taken before the upgrade. |
