...
- **Skip piece boundary safety check**: Enabled by default. If enabled, qui allows cross-seeds even if extra files share torrent pieces with content files. **Warning:** If content differs, this setting can corrupt your existing seeded data. Uncheck this to enable the safety check, or use reflink mode. Reflink mode handles these cases safely.
