...
qui checks a new key when you save it. It sends one request to the tracker. If the tracker refuses the key, qui does not save the settings and shows this message. If the tracker does not answer, qui saves the key and shows a warning that it could not check the key.
