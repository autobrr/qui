...
The path must be absolute on the machine that runs qBittorrent, for example `/data/archive`, `D:\Archive` or `\\nas\media`. qui rejects a relative fixed path or a path that starts with `{{ .Category }}` when you save the rule, and skips the move when a template produces a relative path.
