...
- Linux: every open uses `os.Root` (Go 1.24+) wrapping the allowed-roots directory, which uses `openat2(RESOLVE_BENEATH)` under the hood. This confines all resolution to the root subtree — symlinks that escape the root are rejected — but **does follow** relative symlinks within the root. Mature in Go 1.26.
- macOS / Windows: `os.Root` works with a userspace-resolution fallback. Same confinement semantics.
- Destructive ops (`fs.remove`, `fs.removeall`, `tree.remove`) refuse to operate on the allowed-root itself; the resolved path must be a strict descendant. Additionally, destructive ops must `Lstat` the final path component and reject targets that are symlinks — this prevents an attacker who can create symlinks within the root from redirecting a delete to a different file. The executor enforces this with an explicit `ModeSymlink` check before acting.
