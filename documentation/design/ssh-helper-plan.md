...
2. Implement each FS op in `cmd/qui-helper/internal/executor` (12 ops, one PR each per plan). Destructive ops (`fs.remove`, `fs.removeall`, `tree.remove`) must `Lstat` the final path component and reject symlinks before acting — `os.Root` uses `RESOLVE_BENEATH` only, which follows symlinks within the root (see design doc §6)
...
7. Audit log implementation: path fields must use `strconv.Quote` or `encoding/json.Marshal` — not raw `%s` interpolation — to prevent newline/escape injection from caller-supplied paths

### Deferred Hardening (post-Stage C)

The following were flagged during design review (discussion #1814) and are valid improvements, but not blocking for Stage C. They should be tracked as follow-up work:

1. **AAD-bound encryption for stored secrets.** The existing `encrypt`/`decrypt` helpers across all stores (`instance.go`, `arr_instance.go`, `crossseed.go`, `torznab_indexer.go`) seal with `nil` AAD. Binding AAD to row identity (e.g. `fmt.Appendf(nil, "ssh_creds:v1:instance=%d", id)`) would prevent a DB-write attacker from swapping ciphertext blobs across rows/columns. This is a pre-existing gap, not SSH-specific. In qui's single-user deployment model (attacker with DB write access likely has the encryption key too), the practical risk is low. New SSH columns could use AAD from day one in a follow-up without migration risk since they don't exist in prod yet.
2. **Single-writer guard on the helper.** Two qui installs concurrently SSH-ing to the same seedbox user could race (e.g. one's orphanscan deleting the other's mid-flight hardlink tree). An `flock` on `~/.config/qui-helper/session.lock` at helper startup would give a clear error instead of silent corruption. Low priority — qui is designed as a single install managing all instances, and the local filesystem path has the same unguarded concurrency today.
3. **`authorized_keys command=` language.** §11 presents the `command=` restriction snippet as "Optional but recommended." Strengthening the UI copy to "Strongly recommended — without this, a compromised key grants full shell access" would better reflect the security trade-off. Consider generating a dedicated SSH key pair in qui during deploy to simplify the hardening flow.
