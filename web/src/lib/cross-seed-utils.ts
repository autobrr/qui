...
// Utility function to check if a torrent has cross-seed matches
export const hasCrossSeedMatches = (torrent: Torrent, allTorrents: Torrent[]): boolean => {
  if (!torrent) return false
  
  // Quick check for torrents with same infohash on different instances
  return allTorrents.some(t => 
    t.hash !== torrent.hash && // Different torrent
    (t.infohash_v1 === torrent.infohash_v1 || t.infohash_v2 === torrent.infohash_v2) // Same content
  )
}

// Function to identify potential cross-seed torrents from a list
export const identifyCrossSeedTorrents = (torrents: Torrent[]): Torrent[] => {
  const crossSeedHashes = new Set<string>()
  const infohashGroups = new Map<string, Torrent[]>()
  
  // Group torrents by infohash
  for (const torrent of torrents) {
    const v1 = torrent.infohash_v1
    const v2 = torrent.infohash_v2
    
    if (v1) {
      if (!infohashGroups.has(v1)) {
        infohashGroups.set(v1, [])
      }
      infohashGroups.get(v1)!.push(torrent)
    }
    
    if (v2 && v2 !== v1) {
      if (!infohashGroups.has(v2)) {
        infohashGroups.set(v2, [])
      }
      infohashGroups.get(v2)!.push(torrent)
    }
  }
  
  // Find torrents that appear in groups with multiple members
  for (const [, group] of infohashGroups) {
    if (group.length > 1) {
      for (const torrent of group) {
        crossSeedHashes.add(torrent.hash)
      }
    }
  }
  
  return torrents.filter(t => crossSeedHashes.has(t.hash))
}