      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-green-800 dark:text-green-200">
            {t("ui.updateBanner.title")}
          </p>
          <p className="text-xs text-green-700 dark:text-green-300 mt-1">
            {t("ui.updateBanner.version", { tagName: updateInfo.tag_name })}
          </p>
          <Button
            size="sm"
            variant="outline"
        ...
        >
          <X className="h-3 w-3" />
          <span className="sr-only">{t("ui.updateBanner.dismiss")}</span>
        </Button>
      </div>
    </div>
  );
