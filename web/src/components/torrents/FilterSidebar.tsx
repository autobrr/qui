...
const FilterSidebarComponent = ({
  ...
  const [showDeleteUnusedTagsDialog, setShowDeleteUnusedTagsDialog] =
    useState(false);
  ...
  const filteredCategories = useMemo(() => {
    ...
    const unselectedCategories = categoryEntries.filter(
      ([name]) => !selectedFilters.categories.includes(name)
    );

    if (categoryEntries.length > MAX_INITIAL_ITEMS) {
      const remainingSlots = Math.max(
        0,
        MAX_INITIAL_ITEMS - selectedCategories.length
      );
      return [
        ...selectedCategories,
        ...unselectedCategories.slice(0, remainingSlots),
      ];
    }
  ...
  const filteredTags = useMemo(() => {
    ...
    const unselectedTags = tags.filter(
      (tag) => !selectedFilters.tags.includes(tag)
    );

    if (tags.length > MAX_INITIAL_ITEMS) {
      const remainingSlots = Math.max(
        0,
        MAX_INITIAL_ITEMS - selectedTags.length
      );
  ...
  const filteredTrackers = useMemo(() => {
    ...
    const unselectedTrackers = trackers.filter(
      (tracker) => !selectedFilters.trackers.includes(tracker)
    );

    if (trackers.length > MAX_INITIAL_ITEMS) {
      const remainingSlots = Math.max(
        0,
        MAX_INITIAL_ITEMS - selectedTrackers.length
      );
      return [
        ...selectedTrackers,
        ...unselectedTrackers.slice(0, remainingSlots),
      ];
    }
      ...
      <ScrollArea className="h-full flex-1 overscroll-contain">
        <div className="p-4">
          ...
          >
                    ...
                    <ContextMenu key={name}>
                      <ContextMenuTrigger asChild>
                        <label className="flex items-center space-x-2 py-1 px-2 hover:bg-muted rounded cursor-pointer">
                          ...
                          />
                          <span
                            className="text-sm flex-1 truncate w-8"
                            title={name}
                          >
                            {name}
                          </span>
            ...
            <AccordionItem
              value="trackers"
              className="border rounded-lg last:border-b"
            >
              <AccordionTrigger className="px-3 py-2 hover:no-underline">
                <div className="flex items-center justify-between w-full">
...
export const FilterSidebar = memo(
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      JSON.stringify(prevProps.torrentCounts) ===
        JSON.stringify(nextProps.torrentCounts) &&
