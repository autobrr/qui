...
const zhTWRoot = path.join(localesRoot, "zh-TW")
...
// to leave untranslated in zh-TW.
...
function checkMissingKeys(enFlat, zhFlat, namespace) {
  ...
  // For these, zh-TW only needs _other -- _one is not required.
  ...
  for (const [key, value] of enFlat) {
    // Skip _one keys for v4 plural pairs -- zh-TW collapses to _other only.
...
function checkExtraKeys(enFlat, zhFlat, namespace) {
  ...
  for (const key of zhFlat.keys()) {
    if (!enFlat.has(key)) {
      // If zh-TW has _other and en has _one/_other pair, that is fine.
...
function checkInterpolation(enFlat, zhFlat, namespace) {
  ...
  for (const [key, enValue] of enFlat) {
    ...
    for (const v of enVars) {
      ...
      if (!zhVars.has(v)) {
        errors.push(`${namespace}.${key}: missing {{${v}}} in zh-TW`)
      }
    }
  }
...
function checkTextLength(enFlat, zhFlat, namespace) {
  ...
  for (const [key, enValue] of enFlat) {
    ...
    if (zhClean.length > enClean.length * 1.5) {
      ...
      warnings.push(`${namespace}.${key}: zh-TW is ${ratio}% of en length (${zhClean.length} vs ${enClean.length} chars)`)
    }
  }
...
if (!fs.existsSync(zhTWRoot)) {
  console.log("zh-TW locale directory not found, skipping coverage check.")
  process.exit(0)
}
...
for (const ns of namespaces) {
  ...
  const zhPath = path.join(zhTWRoot, `${ns}.json`)
  ...
  if (!fs.existsSync(zhPath)) {
    errors.missingKeys.push(`${ns}: zh-TW locale file missing`)
    continue
  }
...
if (totalErrors === 0 && totalWarnings === 0) {
  console.log("zh-TW translation coverage: all checks passed.")
  process.exit(0)
}

console.log("=== zh-TW Translation Coverage Report ===\n")
