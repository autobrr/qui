...
- **Gazelle-only run**: A library search with Torznab off; candidates come only from the OPS/RED APIs. Reached by the Torznab switch on the Library card, never inferred from the indexer selection. Needs one Gazelle key; the OPS/RED Torznab indexers leave the picker only when both keys are set. _Avoid_: Torznab-disabled run, forced Gazelle-only.
