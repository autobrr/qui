...
func (sm *SyncManager) AddTorrents(ctx context.Context, instanceID int, files [][]byte, options map[string]string) (*qbt.TorrentAddResponse, error) {
	if len(files) == 0 {
	...
	resp, err := client.AddTorrentsFromMemoryCtx(ctx, files, options)
	if err != nil {
		return nil, err
	}
