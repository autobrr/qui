...
func (s *ruleRunStats) totalApplied() int {
	...
	return s.SpeedApplied + s.ShareApplied + s.PauseApplied + s.TagConditionMet + s.CategoryApplied + s.ExecuteApplied + s.DeleteApplied
}
...
func processRuleForTorrent(rule *models.Automation, torrent qbt.Torrent, state *torrentDesiredState, evalCtx *EvalContext, crossSeedIndex map[crossSeedKey][]qbt.Torrent, stats *ruleRunStats, allTorrents []qbt.Torrent) {
	...
	if conditions.ExecuteExternalProgram != nil && conditions.ExecuteExternalProgram.Enabled {
		...
		if conditions.ExecuteExternalProgram.Condition == nil || conditions.ExecuteExternalProgram.ProgramID == nil {
			if stats != nil {
