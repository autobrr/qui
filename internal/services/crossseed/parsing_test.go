...
// TestDetermineContentTypeForSearch tests the expanded categories for search
func TestDetermineContentTypeForSearch(t *testing.T) {
	tests := []struct {
		name     string
		release  rls.Release
		wantType string
		...
		{
			name:     "Movie with expanded categories",
			release:  rls.Release{Type: rls.Movie, Title: "Test Movie", Year: 2024},
			wantType: "movie",
			wantCats: []int{2000, 2010, 2040, 2050}, // Movies, MoviesSD, MoviesHD, Movies4K
		},
		{
			name:     "TV with expanded categories",
			release:  rls.Release{Type: rls.Episode, Title: "Test Show", Series: 1, Episode: 1},
			wantType: "tv",
			wantCats: []int{5000, 5010, 5040, 5050}, // TV, TVSD, TVHD, TV4K
		},
		{
			name:     "Book with expanded categories",
			release:  rls.Release{Type: rls.Book, Title: "Test Book"},
			wantType: "book",
			wantCats: []int{8000, 8010}, // Books, BooksEbook
		},
		{
			name:     "Music keeps single category",
			release:  rls.Release{Type: rls.Music, Artist: "Test Artist", Title: "Test Album"},
			wantType: "music",
			wantCats: []int{3000}, // Audio (no subcategories)
		},
		{
			name:     "Unknown with Series fallback gets expanded TV categories",
			release:  rls.Release{Type: rls.Unknown, Title: "Test", Series: 1, Episode: 1},
			wantType: "tv",
			wantCats: []int{5000, 5010, 5040, 5050}, // TV categories
		},
		{
			name:     "Unknown with Year fallback gets expanded Movie categories",
			release:  rls.Release{Type: rls.Unknown, Title: "Test", Year: 2024},
			wantType: "movie",
			wantCats: []int{2000, 2010, 2040, 2050}, // Movie categories
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := DetermineContentTypeForSearch(tt.release)
