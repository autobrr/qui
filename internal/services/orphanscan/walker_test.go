...
func TestWalkScanRoot_CollapsesDiscLayoutIntoSingleOrphanUnit(t *testing.T) {
	...
	orphans, truncated, err := walkScanRoot(context.Background(), root, tfm, nil, 0, 100, false)
	if err != nil {
...
func TestWalkScanRoot_DiscUnitSuppressedWhenAnyContainedFileInUse(t *testing.T) {
	...
	orphans, _, err := walkScanRoot(context.Background(), root, tfm, nil, 0, 100, false)
	if err != nil {
		t.Fatalf("walkScanRoot: %v", err)
	}
	if len(orphans) != 0 {
...
func TestWalkScanRoot_IgnorePathSiblingPreventsParentDiscUnit(t *testing.T) {
	...
	tfm := NewTorrentFileMap()
	orphans, truncated, err := walkScanRoot(context.Background(), root, tfm, ignorePaths, 0, 100, false)
	if err != nil {
		t.Fatalf("walkScanRoot: %v", err)
	}
	if truncated {
...
func TestWalkScanRoot_IgnorePathInsideMarkerDisablesDiscGrouping(t *testing.T) {
	...
	tfm := NewTorrentFileMap()
	orphans, _, err := walkScanRoot(context.Background(), root, tfm, ignorePaths, 0, 100, false)
	if err != nil {
		t.Fatalf("walkScanRoot: %v", err)
	}
