...
func TestApplyPathMapping(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			savePath:     "/data/torrents/Movie.Name",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/downloads/Movie.Name",
		},
		{
			name:         "no match falls back to qbitPrefix",
			savePath:     "/other/path/Movie.Name",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/downloads",
		},
		{
			name:         "exact match returns qbitPrefix",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := applyPathMapping(tt.savePath, tt.searcheePath, tt.qbitPrefix)
			if got != tt.want {
				t.Errorf("applyPathMapping(%q, %q, %q) = %q, want %q",
					tt.savePath, tt.searcheePath, tt.qbitPrefix, got, tt.want)
			}
		})
	}
}

func TestApplyInversePathMapping(t *testing.T) {
	tests := []struct {
		name         string
		qbitPath     string
		searcheePath string
		...
		{
			name:         "qbit prefix replaced with host prefix",
			qbitPath:     "/downloads/aither",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/data/torrents/aither",
			wantOK:       true,
		},
		{
			name:         "subdirectory preserved",
			qbitPath:     "/downloads/tv/Show.Name",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/data/torrents/tv/Show.Name",
			wantOK:       true,
		},
		{
			name:         "no match returns qbitPath unchanged",
			qbitPath:     "/other/path/aither",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/other/path/aither",
			wantOK:       false,
		},
		{
			name:         "exact prefix match returns host prefix",
			qbitPath:     "/downloads",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/data/torrents",
		...
		{
			name:         "prefix with similar name does not match",
			qbitPath:     "/downloads-tv/Show.Name",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads",
			want:         "/downloads-tv/Show.Name",
			wantOK:       false,
		},
		{
			name:         "trailing-slash prefix is normalised",
			qbitPath:     "/downloads/aither",
			searcheePath: "/data/torrents",
			qbitPrefix:   "/downloads/",
			want:         "/data/torrents/aither",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, ok := applyInversePathMapping(tt.qbitPath, tt.searcheePath, tt.qbitPrefix)
			if got != tt.want {
				t.Errorf("applyInversePathMapping(%q, %q, %q) = %q, want %q",
					tt.qbitPath, tt.searcheePath, tt.qbitPrefix, got, tt.want)
			}
			if ok != tt.wantOK {
				t.Errorf("applyInversePathMapping(%q, %q, %q) ok = %v, want %v",
					tt.qbitPath, tt.searcheePath, tt.qbitPrefix, ok, tt.wantOK)
			}
		})
	}
}
