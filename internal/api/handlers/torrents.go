...
func torrentFileAddResponseCounts(resp *qbt.TorrentAddResponse, totalCount int) (addedCount int, failedCount int, err error) {
	...
	if addedCount == 1 && failedCount == 0 && resp.PendingCount == 0 && len(resp.AddedTorrentIds) == 0 && totalCount > 1 {
		addedCount = totalCount
	}
	if failedCount > 0 {
		return addedCount, failedCount, errors.New("qBittorrent rejected torrent file")
	}
