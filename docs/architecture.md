...
- `internal/sshpool/`: SSH dialer for remote instances: one-shot dials with the stored key, host-key pinning against the confirmed pin, and the read-only capability probe behind `ssh-test`.
